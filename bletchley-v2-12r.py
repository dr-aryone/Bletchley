...
By Steve Shambles, 31st Jan 2019.
...
# changes\additions made by shambles for V2.12r
#---------------------------------------------------------

# Added win rate percentage. First time in my life used float in a program lol
# had to find a way to round it to 1 decimal place, used 'round' func. tx google.

# Had to seperate reset_stats def into 2 sep defs to allow for file error
# as ask yn msgbox comes up oterwise, that would be ok, but if player clicked no
# would crash as needed to reset to zero always in this case, different to
# manual reset stats.

# Final problem is now to get row players has won game on (row_index).
...
class GameWindow(object):
    ...
    def new_game(self):
        ...
        self.MAIN_MENU.add_command(
            ...
            command=partial(
                showinfo,
                "About",
                "Bletchley V2.12r by Steve Shambles jan 2019",
            ),
        )
        ...
        self.HEADER_FRAME = LabelFrame(
            self.ROOT, fg="blue", text="Bletchley V2.12r"
        )
    ...
    def reset_stats(self):
        ...
        QUEST = messagebox.askyesno('Really?', 'Are you sure you want to reset all your stats to zero?')
    ...
    def calc_points_won(self):
        '''Do simple calc for points won depending on how many tries it took to win.'''

        #i dont know how to get row_index value yet
        return

        pts = row_index * 10
        points = 70 - pts
        PlayerStats.points = PlayerStats.points + points
        print (PlayerStats.points)#remove later

    def calc_win_rate(self):
        '''calculate percent wins to games ratio'''
        PlayerStats.win_rate = PlayerStats.wins  / PlayerStats.played *100
        print (PlayerStats.win_rate)
    ...
    def load_stats(self):
        ...
        with open("bletchley.blt", 'r') as contents:
            ...
            print("points=", PlayerStats.points)#remove later

            PlayerStats.win_rate = float((all_stats[4]))
            PlayerStats.win_rate = round(PlayerStats.win_rate, 1)
            print("win_rate=", PlayerStats.win_rate)#remove later
